# Version 44.0 | File: ui_main.py | Main Dashboard and Event Loop
import os
import sys
import threading
import queue
...
class ModernDNSApp(ctk.CTk):
    ...
    def build_ui(self):
        ...
        # Option B: Clean Separation Progress Bar
        ...
        self.btn_prefs = ctk.CTkButton(right_frame, text="⚙️", font=("Segoe UI Emoji", 24), command=self.open_preferences, fg_color="transparent", text_color=C_TEXT_MAIN, hover_color=C_BORDER, corner_radius=24, width=44, height=44)
    ...
    def load_history_data(self, selected_criteria, df):
        ...
        grouped = filtered_df.groupby(['DNS_IP', 'DNS_Name']).agg(
            Test_Count=('Errors', 'count'), Avg_Errors=('Errors', 'mean'), Avg_Ping=('Avg_Ping_ms', lambda x: x[x > 0].mean() if (x > 0).any() else -1)
        ).reset_index()
    ...
    def build_grid(self):
        ...
        if self.display_mode == "history":
            ...
            for row_index, row_data in enumerate(self.history_results):
                ...
                values = [row_index + 1, row_data['DNS_IP'], row_data['DNS_Name'], err_val, ping_str, int(row_data['Test_Count'])]
        ...
        for row_index, info in enumerate(self.dns_list):
            ...
            values = [row_index + 1, addr_text, info["name"], err_text, avg] + ["-"] * len(self.domains)
            self.tree_items[row_id] = self.table_scroll.tree.insert("", "end", iid=row_id, values=values)
